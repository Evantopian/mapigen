...
from mapigen.models import Parameter
...
def _get_raw_parameter_details(param: dict[str, Any]) -> dict[str, Any]:
    """Extracts the raw details of a parameter, preserving all original fields."""
    return param.copy()
...
def _unwrap_schema_properties(schema: dict[str, Any], spec: dict[str, Any], visited: set[str]) -> list[dict[str, Any]]:
    ...
    for name, prop_data_item in schema.get("properties", {}).items():
        ...
        if "$ref" in prop_data:
            ...
            param_details.update(_get_raw_parameter_details(prop_data))
...
def get_params_from_operation(op_details: dict[str, Any], path_params: list[dict[str, Any]], spec: dict[str, Any]) -> list[Parameter]:
    ...
    for media_type in content.values():
        if "schema" in media_type:
            # For request bodies, the required status is on the requestBody itself
            is_required = request_body.get("required", False)
            unwrapped_params = _unwrap_schema_properties(media_type["schema"], spec, set())
            for param in unwrapped_params:
